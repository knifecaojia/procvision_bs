  ...
  <div class="app-container">
    ...
    <el-table ref="operlogRef" v-loading="loading" :data="operlogList" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
      ...
      <el-table-column label="审核状态" align="center" prop="auditStatus">
        <template #default="scope">
          <el-tag v-if="scope.row.title === '操作日志'" type="info">无需审核</el-tag>
          <template v-else>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-button link type="success" icon="Edit" @click="handleAudit(scope.row)"
                     v-hasPermi="['monitor:operlog:audit']"
                     v-if="scope.row.auditStatus === '0' && scope.row.operName !== userStore.name && scope.row.title !== '操作日志'">
            审核</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="操作日志审核" v-model="auditVisible" width="500px" append-to-body>
      <el-form ref="auditRef" :model="auditForm" :rules="auditRules" label-width="80px">
        ...
        <el-form-item label="审核意见" prop="auditMsg">
          <el-input v-model="auditForm.auditMsg" type="textarea" placeholder="请输入审核意见" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import {list, delOperlog, cleanOperlog, auditOperlog} from "@/api/monitor/operlog"
...
function submitAudit() {
  proxy.$refs["auditRef"].validate(valid => {
    if (valid) {
      auditOperlog(auditForm.value).then(response => {
